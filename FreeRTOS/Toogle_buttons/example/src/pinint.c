...
#define GPIO_INTERRUPT_PIN_A     17	/* GPIO pin number mapped to interrupt */
#define GPIO_INTERRUPT_PIN_B     18
...
bool Board_ZButtons_Test(int button)
{
	bool state = false;

	if (button == 0)
	{
		state = Chip_GPIO_ReadPortBit(LPC_GPIO, GPIO_INTERRUPT_PORT, GPIO_INTERRUPT_PIN_A);
	}
	else if (button == 1)
	{
		state = Chip_GPIO_ReadPortBit(LPC_GPIO, GPIO_INTERRUPT_PORT, GPIO_INTERRUPT_PIN_B);
	}
	return state;
}
...
{
	Chip_GPIOINT_ClearIntStatus(LPC_GPIOINT, GPIO_INTERRUPT_PORT, (1 << GPIO_INTERRUPT_PIN_A)|(1 << GPIO_INTERRUPT_PIN_B));

	bool stateButtonA = Board_ZButtons_Test(0);
	bool stateButtonB = Board_ZButtons_Test(1);
...
int main(void)
{
	...
	Chip_GPIO_SetPinDIRInput(LPC_GPIO, GPIO_INTERRUPT_PORT, GPIO_INTERRUPT_PIN_A);
	Chip_GPIO_SetPinDIRInput(LPC_GPIO, GPIO_INTERRUPT_PORT, GPIO_INTERRUPT_PIN_B);

	/* Configure the GPIO interrupt */
	Chip_GPIOINT_SetIntFalling(LPC_GPIOINT, GPIO_INTERRUPT_PORT, (1 << GPIO_INTERRUPT_PIN_A)|(1 << GPIO_INTERRUPT_PIN_B));
