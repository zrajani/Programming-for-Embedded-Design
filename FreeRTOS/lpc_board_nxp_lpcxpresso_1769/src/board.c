...
void Board_LED_Set(uint8_t LEDNumber, bool On) {
	...
	if (LEDNumber == 0) {
		Chip_GPIO_WritePortBit(LPC_GPIO, LED0_GPIO_PORT_NUM, LED0_GPIO_BIT_NUM,
				On);
	}
	if (LEDNumber == 1) {
		Chip_GPIO_WritePortBit(LPC_GPIO, LED1_GPIO_PORT_NUM, LED1_GPIO_BIT_NUM,
				On);
	}
	if (LEDNumber == 2) {
		Chip_GPIO_WritePortBit(LPC_GPIO, LED2_GPIO_PORT_NUM, LED2_GPIO_BIT_NUM,
				On);
	}
}
